...
export class RateService {
  ...
  async fetchRates(): Promise<void>{ 
    ...
    try{
      ...
      this.cacheRates(response.data); //updates cache
  ...
  private cacheRates(rates: any): void {
    ...
    Object.keys(rates).forEach(coin => {
      this.cachedCoinRates[coin] = { ...this.cachedCoinRates[coin], ...rates[coin] };
    });
  ...
  async restoreSupportedData(): Promise<void> {
    try{
      ...
      this.logger.log(`Supported coins and currencies restored from file (Last saved: ${restoredAt})`);
    }
    catch(error){
      this.logger.error('Failed to restore supported coins and currencies from file', error);
    }
  }
  ...
  async getCryptoPrice(coin: string, currency: string = 'usd', skipCache: boolean = false): Promise<number>{
    ...
    try{
      ...
      if (rate){
        ...
        this.cachedCoinRates[coin] = { ...this.cachedCoinRates[coin], [currency]: rate };
      } 
      return rate; 
    }catch(error){
      if (error.response?.status === 429) {
